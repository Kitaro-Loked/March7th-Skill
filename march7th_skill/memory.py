...
class EmotionMemory:
    ...
    def generate_diary(self) -> str:
        """生成日记摘要"""
        if not self._memories:
            return "*(翻开日记本)* 嗯……今天还没有什么记录呢。让我们一起去创造些回忆吧！📸"
        
        recent = self.get_recent(self.max_entries)
        form_freq = self.get_form_frequency()
        most_used_form = max(form_freq, key=form_freq.get) if form_freq else "存护"
        ...
        top_keywords = list(set(all_keywords))[:5] if all_keywords else ["冒险"]
        
        diary = (
            f"*(坐在窗边，轻轻翻开日记本，笔尖在纸上沙沙作响)*\n\n"
            f"**三月七的日记** 📖\n"
            f"日期：{datetime.now().strftime('%Y年%m月%d日')}\n"
            f"天气：{'晴朗' if emotion_trend > 0 else '多云' if emotion_trend == 0 else '微雨'}\n\n"
            f"今天和开拓者聊了 {len(recent)} 次天。"
            f"大多数时候我是「{most_used_form}」形态。\n"
            f"我们聊了很多关于「{'、'.join(top_keywords)}」的话题。\n"
        )
        
        if emotion_trend > 0.3:
            diary += "今天真的很开心！和开拓者在一起的每一刻都值得记录。✨\n"
        elif emotion_trend < -0.3:
            diary += "今天有些心事……但没关系，有开拓者在身边，一切都会好起来的。\n"
        else:
            diary += "平平淡淡的一天，但和开拓者在一起，就算平凡也闪闪发光。\n"
        
        # 引用一条具体记忆
        if recent:
            memorable = recent[-1]
            diary += (
                f"\n印象最深的是开拓者说：「{memorable.user_message[:40]}……」\n"
