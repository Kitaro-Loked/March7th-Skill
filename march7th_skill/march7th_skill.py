...
class March7thMultiverse(BaseSkill):
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.name: str = self._skill_config.get("name", "March7th_Multiverse_Skill")
        self.description: str = self._skill_config.get(
            "description", "三月七·全纪实灵魂觉醒"
        )
        self.version: str = self._skill_config.get("version", "3.0.0")
        self.current_form: str = self._skill_config.get("default_form", "存护")
        ...
        logger.info(
            f"March7thMultiverse v{self.version} initialized. "
    ...
    def _get_localized_text(self, key: str, **kwargs: Any) -> str:
        ...
        texts = {
            "zh": {
                "unknown_form": "欸？'{form}'是什么形态呀，本姑娘没听说过呢~\n请输入：切换形态 存护 / 寻猎 / 长夜月",
                ...
                "diary_not_ready": "*(翻着日记本)* 唔……对话还不够多呢，再聊一会儿再写日记吧！",
            ...
            "en": {
                "unknown_form": "Huh? I've never heard of the '{form}' form~\nPlease use: switch form Preservation / Hunt / Eternal Night Moon",
                "switch_hint": "Huh? Which form do you want to switch to? Please use:\n`switch form Preservation` / `switch form Hunt` / `switch form Eternal Night Moon`",
                ...
                "help_text": (
                    "**March 7th: Complete Chronicle Soul Awakening v3.0** ✨\n\n"
                    "📋 Command List:\n"
    ...
    async def handle_message(self, ctx: Any) -> None:
        ...
        try:
            ...
            # 记录记忆（在响应后由调用方补充）
            # 这里只设置上下文，实际记忆在响应后记录
            ctx._march7th_form = self.current_form
            ctx._march7th_memory = self._memory
    ...
    async def cmd_minigame(self, ctx: Any) -> None:
        ...
        try:
            ...
            if args in ["拍照", "photo", "写真", "photography"]:
                await ctx.send(self._minigame_manager.play_photo_game(self.language))
            elif args in ["剑术", "sword", "剣術", "sword practice"]:
                await ctx.send(self._minigame_manager.play_sword_game(self.language))
            else:
                games = self._minigame_manager.list_games(self.language)
    ...
    async def cmd_help(self, ctx: Any) -> None:
        ...
        try:
            ...
            await ctx.send(self._get_localized_text("help_text", prob=prob))
        except Exception as e:
